...
| 4 | Cross-artifact drift (mirror R vs crosswalk/lookup/projection R-1) | **Release-match gate:** activate R only when the mirror, `TherapyOmopMapping`, `ComponentCategoryOmopLookup`, and the trial `omop_*` projection all agree on R. Mismatch → do not activate, keep serving the last consistent bundle, alert. |
