...
## Problem
EXACT (trial matcher) and SoC (therapy recommendation engine) must match patients whose data arrives in CTOMOP/OMOP vocabularies (HemOnc regimens, RxNorm drugs) against trials authored in CancerBot's internal codes — on a shared vocabulary, so a stateless EXACT and external patient sources interoperate.
...
The design:
- **CB owns the crosswalk.** Built in CB by `build_mapping.py` (name-match Athena/CTOMOP dumps + curated overrides + `NO_OMOP`; LLM rows merged separately) → CSV (`match` provenance: auto / curated / llm / needs_review / no_omop) → `TherapyOmopMapping` (1:1 per `(level, cb_code)`) + `OmopConcept`. EXACT vendors only the CSV.
...
1. SME-curate the `needs_review` codes; pin a CTOMOP release.
2. **Supportive patient-side conversion:** make `normalize_ctomop_row` emit supportive concept_ids (or revert the profile's supportive flip) so the supportive seam is consistent end-to-end — see the code-inconsistency note.
...
5. **Demographics read-cutover:** deferred until CTOMOP emits race/gender concept_ids; a `DemographicsMatchProfile` OMOP variant + the flag flip mirror the therapy seam when it does.
...
- Legacy name-string bridge bugs (#174) — clinical-safety, on the `ctomop_adapter` path, separate from the OMOP concept path.
...
- But `ctomop_adapter.py::normalize_ctomop_row` does **not** convert `supportive_therapies` (its comment even asserts "the OMOP profile keeps supportive/planned legacy" — stale/false w.r.t. the profile), and `therapy_concept_mapper.py` says matching "still reads the legacy supportive columns until the coordinated flip."

So the trial-side flip landed ahead of the patient-side conversion. Because `EXACT_OMOP_THERAPY` defaults OFF and the #221 gate blocks the prod flip, this is not live-breaking, but under the flag it silently drops supportive constraints. Reconcile by either (a) adding supportive concept_id emission to `normalize_ctomop_row` before the flip, or (b) reverting the profile's supportive flip until the adapter is ready — and fix the stale comments either way.
