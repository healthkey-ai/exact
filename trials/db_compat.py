...
def install(app_config):
    ...
    Doing this per-view does not hold: the corpus is behind the models on more
    than one table — `trials_therapy.omop_concept_id` is missing too, so the
    form-settings endpoint fails the same way `/trials/` did, and so would the
    next endpoint over whichever table drifts next. Patching at the manager
    level covers `Model.objects` queries across the app from one place.
    ...
    Related managers (`instance.related_set`) are built dynamically from the
    model class and are not covered. That is acceptable here — the corpus
    drift is on tables reached directly — but it is a reason this is a
    stopgap and not an abstraction to build on. See exact#360.
    """
    if not getattr(settings, 'TRIALS_DB_TOLERATE_MISSING_COLUMNS', False):
        return
    ...
    for model in app_config.get_models():
        for manager in model._meta.managers:
            ...
            def get_queryset(self, _original=original):
                return defer_missing_columns(_original())

            manager.get_queryset = types.MethodType(get_queryset, manager)
            manager._defers_missing_columns = True
