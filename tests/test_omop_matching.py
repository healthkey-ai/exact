...
# ── queryset: eligible_for_therapy_related_things_from_lines OMOP path ──

@override_settings(EXACT_OMOP_THERAPY=True)
def test_queryset_component_concept_ids_none_returns_trial_fail_open():
    """component_concept_ids=None → fail-open: trial requiring OMOP component still returned."""
    trial = TrialFactory(
        omop_therapies_required=['111'],
        omop_therapy_components_required=['900825'],
    )
    qs = Trial.objects.eligible_for_therapy_related_things_from_lines(
        therapy_codes=['111'],
        component_concept_ids=None,
    )
    assert trial.id in set(qs.values_list('id', flat=True))


@override_settings(EXACT_OMOP_THERAPY=True)
def test_queryset_component_concept_ids_match_returns_trial():
    """component_concept_ids with matching cid → trial returned."""
    trial = TrialFactory(
        omop_therapies_required=['111'],
        omop_therapy_components_required=['900825'],
    )
    qs = Trial.objects.eligible_for_therapy_related_things_from_lines(
        therapy_codes=['111'],
        component_concept_ids=['900825'],
    )
    assert trial.id in set(qs.values_list('id', flat=True))


@override_settings(EXACT_OMOP_THERAPY=True)
def test_queryset_component_concept_ids_mismatch_excludes_trial():
    """component_concept_ids with no matching cid → trial requiring component excluded."""
    trial = TrialFactory(
        omop_therapies_required=['111'],
        omop_therapy_components_required=['900825'],
    )
    qs = Trial.objects.eligible_for_therapy_related_things_from_lines(
        therapy_codes=['111'],
        component_concept_ids=['999999'],
    )
    assert trial.id not in set(qs.values_list('id', flat=True))
