...
export default defineConfig(function (_a) {
    ...
    var apiProxyTarget = env.VITE_EXACT_API_PROXY_TARGET || "http://localhost:8000";
    ...
    // ports cleanly.
    var ctomopLocalTarget = env.VITE_CTOMOP_LOCAL_TARGET || "http://localhost:8001";
    var ctomopStagingTarget = env.VITE_CTOMOP_STAGING_TARGET || "https://ctomop.onrender.com";
    ...
    var makeCookieRewriter = function (mountPath) {
        var rewriteCookie = function (raw) {
            return raw
                .replace(/;\s*Secure/gi, "")
                .replace(/;\s*SameSite=None/gi, "; SameSite=Lax")
                .replace(/(;\s*Path=)\/(?=;|$)/gi, "$1".concat(mountPath));
        };
        ...
        server: {
            ...
            proxy: {
                ...
                "/ctomop-local": {
                    ...
                    changeOrigin: true,
                    rewrite: function (p) { return p.replace(/^\/ctomop-local/, ""); },
                    configure: makeCookieRewriter("/ctomop-local"),
                },
                "/ctomop-staging": {
                    target: ctomopStagingTarget,
                    changeOrigin: true,
                    rewrite: function (p) { return p.replace(/^\/ctomop-staging/, ""); },
